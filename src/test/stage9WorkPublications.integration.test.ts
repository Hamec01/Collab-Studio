...
test("work publication snapshots current lyrics, serves public audio and archives cleanly", async () => {
  ...
  const createResponse = await apiJson<{ publication: { slug: string; snapshotId: string; projectId: string } }>("/api/publications/works", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
      projectId,
  ...
  const publicResponse = await apiJson<{ work: Record<string, unknown> }>(`/api/public/works/${slug}`);
  assert.equal(publicResponse.status, 200);
  const work = publicResponse.body.work as {
    title: string;
  ...
  assert.equal(work.audio?.streamUrl, `/api/public/works/${slug}/stream`);
  assert.equal(work.audio?.downloadUrl, `/api/public/works/${slug}/download`);
  assert.equal("projectId" in publicResponse.body.work, false);
  assert.equal("trackId" in publicResponse.body.work, false);
  assert.equal("snapshotId" in publicResponse.body.work, false);

  const streamResponse = await fetch(`http://127.0.0.1:${appPort}/api/public/works/${slug}/stream`);
  ...
  const rangeResponse = await fetch(`http://127.0.0.1:${appPort}/api/public/works/${slug}/stream`, {
  ...
  const hiddenAfterArchive = await apiJson<{ error: { code: string } }>(`/api/public/works/${slug}`);
...
test("outsider cannot create a publication for a foreign project", async () => {
  ...
  const denied = await apiJson<{ error: { code: string } }>("/api/publications/works", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
